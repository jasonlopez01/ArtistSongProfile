...
function RadarChart(id, data, options) {
	...
	d3.select(".radarLegend").select("svg").remove();
	...
	blobWrapper
		...
		.attr("class", function(d) {
			return "radarArea" + " " + d[0][areaName].replace(/\s+|\W/g, '') //Remove spaces from the areaName string to make one valid class name
		})
		...
		.on('mouseover', function (d,i){
		    ...
		    //highlight legend
            d3.select(".legendOrdinal").selectAll('g:nth-child(' + (i+1) + ')')
                .transition().duration(200)
                .style('opacity', 0.2)
		})
		.on('mouseout', function(){
			...
			d3.selectAll(".radarArea")
				...
				.style("fill-opacity", cfg.opacityArea);
            //un-highlight legend
            d3.select(".legendOrdinal").selectAll('g')
                .transition().duration(200)
                .style('opacity', 1.0)
		})
	...
	function wrap(text, width) {
	  text.each(function() {
		var text = d3.select(this),
			words = text.text().split(/\s+|\W/).reverse(),
			word,
			line = [],
			lineNumber = 0,
			lineHeight = 1.4, // ems
	...
	function cellover(d) {
	        //highlight
            d3.select(".legendOrdinal").selectAll('g:nth-child(' + (d+1) + ')')
                .transition().duration(200)
                .style('opacity', 0.2)
                .style('cursor', 'pointer')
			//Dim all blobs
	...
	//svg.append("g") changed to add in different element
	legendsvg.append('g')
